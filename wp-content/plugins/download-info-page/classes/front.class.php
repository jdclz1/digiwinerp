...
//		if(!is_single()) return;
	...
	public function wp_head(){
        ...
        if ( is_single() && $with_dl_info ) {
	        ...
	        wp_enqueue_script('wbs-front-dlipp', plugin_dir_url(DLIPP_BASE_FILE) . 'assets/wbp_front.js', array('clipboard-js'), DLIPP_VERSION,true);
...
//	        $is_comment = $this->wb_is_comment($post_id);
	...
	public static function wb_is_comment($post_id){
		...
		global $wpdb;
		$pid = $post_id;
		$query = "SELECT `comment_ID` FROM {$wpdb->comments} WHERE `comment_post_ID`={$pid} and `comment_approved`='1' and `comment_author_email`='{$email}' LIMIT 1";
